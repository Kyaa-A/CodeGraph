...
export async function POST(request: NextRequest) {
  try {
    ...
    const { success } = rateLimit(`execute:${user.id}`, 20, 60_000);
    if (!success) {
    ...
    // Java: Wandbox expects non-public class or filename match.
    // Replace "public class" with "class" so it compiles.
    if (language === "java") {
...
export async function GET() {
  const languages = Object.entries(COMPILER_MAP).map(([id, compiler]) => ({
    id,
